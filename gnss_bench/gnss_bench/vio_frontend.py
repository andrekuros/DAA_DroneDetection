...
def process_run(run_dir: Path, min_baseline_m: float, debug_dir: Path | None,
    ...
    # Descarta linhas sem GT ou sem JPEG (captura parcial / crash do Unreal).
    usable = []
    for i, row in enumerate(rows):
        try:
            _gt_xyz(row)
            fp = _frame_path(run_dir, row)
        ...
        usable.append((i, row))
    if len(usable) < 2:
        raise ValueError(f"{run_dir.name}: menos de 2 frames usaveis em frames.csv")

    anchor_i, anchor_row = usable[0]
    anchor_xyz = _gt_xyz(anchor_row)
    anchor_img = cv2.imread(str(_frame_path(run_dir, anchor_row)), cv2.IMREAD_GRAYSCALE)

    for j, row in usable[1:]:
        xyz = _gt_xyz(row)
