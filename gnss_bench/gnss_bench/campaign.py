...
from gnss_bench.paths import REPO_ROOT, px4_dir, sim_exe
...
def airsim_reset(ip: str, vehicle: str) -> None:
    ...
    """
    try:
        from gnss_bench.airsim_gt import AirSimGroundTruth
...
def sim_stop() -> None:
    """Encerra o Unreal/Cosys-AirSim."""
    for name in ("CitySample-Win64-Shipping.exe", "CitySample.exe"):
        subprocess.run(["taskkill", "/F", "/IM", name],
                       capture_output=True, timeout=30)


def sim_start(exe: str = SIM_EXE, timeout_s: float = 480.0) -> None:
    """Sobe o Unreal e espera a porta do lockstep abrir."""
    subprocess.Popen(
    ...
    while time.monotonic() < deadline:
        ...
        if any(":4560" in ln and "LISTENING" in ln for ln in r.stdout.splitlines()):
            print("[OK] Cosys-AirSim pronto.")
            time.sleep(10.0)  # deixa a cena assentar antes de conectar
            return
        time.sleep(5.0)
    raise TimeoutError(f"Cosys-AirSim nao abriu a porta 4560 em {timeout_s:.0f}s.")


def full_restart(log_path: Path, exe: str = SIM_EXE) -> None:
    """
    Reinicia Unreal + PX4 juntos.

    Necessario em campanhas longas: o cliente de parametros do MAVSDK degrada com
    o tempo de sessao e a negacao de GNSS passa a dar TIMEOUT — observado falhando
    3 vezes seguidas num mesmo voo. Reiniciar so o PX4 nao resolve (deixa o AirSim
    preso na sessao TCP anterior, ver px4_restart), entao os dois vao juntos.
    """
    print("\n[INFO] Reinicio completo (Unreal + PX4) para renovar a sessao ...")
    px4_stop()
    sim_stop()
    time.sleep(6.0)
    sim_start(exe)
...
def run_is_reusable(out_dir: Path, run_name: str) -> bool:
    """
    True se o run ja existe, terminou ok e nao colidiu.

    Usado com --skip-existing para retomar uma campanha (ex.: 3 denied prontos
    e completar ate 30) sem sobrescrever pastas validas.
    """
    run_dir = out_dir / run_name
    meta_path = run_dir / "meta.json"
    if not meta_path.is_file() or not (run_dir / "telemetry.csv").is_file():
        return False
    try:
    ...
    if meta.get("status") != "ok":
        return False
    if meta.get("collision", {}).get("occurred"):
        return False
    return True
...
def run_flight(run_name: str, out_dir: Path, deny_at_m: float, args,
    ...
    cmd = [
        sys.executable, "-u", "-m", "gnss_bench.cli", "fly",
        "--run-name", run_name,
        "--out-dir", str(out_dir),
...
def aggregate(out_dir: Path) -> dict:
    ...
    from gnss_bench.analyze import aggregate_campaign
...
def main() -> None:
    ...
    ap.add_argument("--restart-every", type=int, default=5, metavar="N",
                    help="Reinicia Unreal+PX4 a cada N voos executados (0 desativa). "
                         "Necessario em campanhas longas: o cliente de parametros do "
                         "MAVSDK degrada e a negacao de GNSS passa a dar TIMEOUT.")
    ap.add_argument("--sim-exe", default=SIM_EXE, metavar="PATH",
                    help="Executavel do Cosys-AirSim / CitySample")
    ap.add_argument("--skip-existing", action="store_true",
                    help="Pula runs cujo meta.json ja esta ok sem colisao "
                         "(retoma campanha sem sobrescrever)")
    args = ap.parse_args()
    ...
    for kind, deny, n in plan:
        ...
        while i < n and attempts < n + args.max_retries:
            ...
            # Vaivem pelo indice do rep concluido (nao pela tentativa): skips e
            # retries de colisao nao embaralham o corredor Norte/Sul.
            direction = 1 if (i % 2 == 0) else -1

            if args.skip_existing and run_is_reusable(out_dir, run_name):
                print(f"[INFO] Pulando {run_name} (ja existe e e valido)")
                results.append({"run": run_name, "kind": kind, "deny_at_m": deny,
                                "ok": True, "collided": False, "used": True,
                                "skipped": True})
                i += 1
                continue
            ...
            attempts += 1
            try:
                # Renovar a sessao periodicamente, ou assim que dois voos
                # seguidos falharem — a degradacao do cliente de parametros do
                # MAVSDK se manifesta como falha repetida na negacao, e insistir
                # sem reiniciar so gasta o orcamento de retry.
                need_restart = (
                    args.restart_every > 0
                    and flights_done > 0
                    and flights_done % args.restart_every == 0
                ) or consecutive_fail >= 2
                if need_restart:
                    if consecutive_fail >= 2:
                        print(f"[INFO] {consecutive_fail} falhas seguidas; renovando a sessao.")
                    full_restart(px4_log, args.sim_exe)
                    consecutive_fail = 0
                elif args.restart_px4:
